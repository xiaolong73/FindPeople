...
class Market1501_test(BaseImageDataset):
    ...
    def __init__(self, root='query', verbose=True, **kwargs):
        super(Market1501_test, self).__init__()
    ...
    def _process_dir(self, dir_path, relabel=False):
        """
        对 dir_path 的数据进行处理
        :param dir_path: 'query'
        :param relabel:  训练时：True 测试时：False
        :return:
        """
        # 列表: 含有 n 个图片的路径
        # 'query\\0003_c1s6_015971_00.jpg'
        img_paths = glob.glob(osp.join(dir_path, '*.jpg'))
        ...
        for img_path in img_paths:
            ...
            pid, _ = map(int, pattern.search(img_path).groups())
            if pid == -1:
                continue  # junk images are just ignored 测试集 gallery 会有-1的图片，扔掉
        ...
        for img_path in img_paths:
            pid, camid = map(int, pattern.search(img_path).groups())
            if pid == -1:
                continue  # junk images are just ignored pid为-1代表垃圾图片，忽略掉
